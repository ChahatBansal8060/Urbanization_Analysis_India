...
for cluster_id in range(len(final0_clusters_feature_vectors)):
    print("**********Major Cluster0- Subcluster: ",cluster_id,"***********")
...
# Print average values of each subcluster
for cluster_id in range(len(final1_clusters_feature_vectors)):
    print("**********Major Cluster1- Subcluster: ",cluster_id,"***********")
...
Major cluster 0:
    ...
    sub-cluster 3 = Class-I (1)
'''
# re-labeling sub-clusters of major cluster-0
for i in range(len(final0_clusters_feature_vectors)):
    if i == 0 or i == 1:
        final0_clusters_grid_info[i]["Class_label"] = [2] * len(final0_clusters_grid_info[i])
    elif i == 2:
        final0_clusters_grid_info[i]["Class_label"] = [3] * len(final0_clusters_grid_info[i])
    elif i == 3:
        final0_clusters_grid_info[i]["Class_label"] = [1] * len(final0_clusters_grid_info[i])
...
Some clusters will be merged and others will be re-labelled
Major cluster 1:
    sub-cluster 0 = Class-V (5)
    sub-cluster 1 = Class-III (3)
    sub-cluster 2 + sub-cluster 3 = Class-IV (4) 
'''
# re-labeling sub-clusters of major cluster-1
for i in range(len(final1_clusters_feature_vectors)):
    if i == 0:
        final1_clusters_grid_info[i]["Class_label"] = [5] * len(final1_clusters_grid_info[i])
    elif i == 1:
        final1_clusters_grid_info[i]["Class_label"] = [3] * len(final1_clusters_grid_info[i])
    elif i == 2 or i == 3:
        final1_clusters_grid_info[i]["Class_label"] = [4] * len(final1_clusters_grid_info[i])

# Merge desired clusters as per their labels
final_clusters = []
# Class 1
final_clusters.append( final0_clusters_feature_vectors[3].copy() )
# Class 2
final_clusters.append( np.append(final0_clusters_feature_vectors[0], final0_clusters_feature_vectors[1].copy(), axis=0) )
# Class 3
final_clusters.append( np.append(final0_clusters_feature_vectors[2], final1_clusters_feature_vectors[1].copy(), axis=0) )
# Class 4
final_clusters.append( np.append(final1_clusters_feature_vectors[2], final1_clusters_feature_vectors[3].copy(), axis=0) )
# Class 5
final_clusters.append( final1_clusters_feature_vectors[0].copy() )
