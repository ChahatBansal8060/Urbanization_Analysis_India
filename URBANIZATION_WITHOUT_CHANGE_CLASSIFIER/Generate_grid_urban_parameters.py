        ...
        for col_id in range(image_cols):
            ...
            for u in range( row_id-ellipse_a, row_id+ellipse_a +1, 1):
                for v in range( col_id-ellipse_b, col_id+ellipse_b +1, 1):
                    ...
                    # if the pixel is within the boundary of ellipse ((x-i)^2 b^2)+((y-j)^2 a^2)-(a^2 b^2) <= 0
                    if ((u-row_id)**2 * ellipse_b**2) + ((v-col_id)**2 * ellipse_a**2) - (ellipse_a**2 * ellipse_b**2) <= 0:
                        if BU_NBU_map1[u][v] == 65:
                            BU_pixels_WDC1 += 1
                        if BU_NBU_map2[u][v] == 65:
                            BU_pixels_WDC2 += 1
...
for district in districts:
    ...
    BU_NBU_image_main_directory = 'BU_NBU_maps/'+district
    BU_NBU_filepath_first_year = BU_NBU_image_main_directory+'/'+district+'_BU_NBU_2016.png'
    BU_NBU_filepath_last_year = BU_NBU_image_main_directory+'/'+district+'_BU_NBU_2019.png'

    BU_NBU_first_year = np.array( Image.open(BU_NBU_filepath_first_year) )
    BU_NBU_last_year = np.array( Image.open( BU_NBU_filepath_last_year) )

    # Pad the image to fill gap between tight bounds and rounded bounds
    padded_BU_NBU_first_year = Pad_image(BU_NBU_first_year, min_lat, max_lat, min_lon, max_lon, rounded_min_lat, rounded_max_lat, rounded_min_lon, rounded_max_lon)
    padded_BU_NBU_last_year = Pad_image(BU_NBU_last_year, min_lat, max_lat, min_lon, max_lon, rounded_min_lat, rounded_max_lat, rounded_min_lon, rounded_max_lon)
    ...
    '''
    Starting with grid-level analysis
    '''
